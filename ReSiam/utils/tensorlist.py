...
class TensorList(list):
    ...
    def unroll(self):
        if not any(isinstance(t, TensorList) for t in self):  # TensorList中有1个元素是TensorList就需要进行展开
            return self
        ...
        for t in self:
            if isinstance(t, TensorList):
                ...
                new_list.append(t)
        return new_list  # 得到展开后的TensorList，保证数据为TensorList[如Tensor]
...
def tensor_operation(op):
    ...
    def oplist(*args, **kwargs):
        ...
        if len(args) == 1:
            ...
            # Multiple operands, assume max 5
            if len(args) > 4 and islist(args[0]) and islist(args[1]) and islist(args[2]) and islist(args[3]) and islist(args[4]):
                return TensorList([op(a, b, c, d, e, *args[5:], **kwargs) for a, b, c, d, e in zip(*args[:5])])
            if len(args) > 3 and islist(args[0]) and islist(args[1]) and islist(args[2]) and islist(args[3]):
                return TensorList([op(a, b, c, d, *args[4:], **kwargs) for a, b, c, d in zip(*args[:4])])
            if len(args) > 2 and islist(args[0]) and islist(args[1]) and islist(args[2]):
                return TensorList([op(a, b, c, *args[3:], **kwargs) for a, b, c in zip(*args[:3])])
