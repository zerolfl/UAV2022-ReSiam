...
from .image_loader import imread_indexed
...
class Sequence:
    ...
    def init_info(self):
        info = self.frame_info(frame_num=0)  # 初始化的帧信息Dict类，包含了bbox等信息
        return info

    def frame_info(self, frame_num):
        info = self.object_init_data(frame_num=frame_num)  # 获得指定帧信息（Dict类），包含了bbox等信息
        return info
