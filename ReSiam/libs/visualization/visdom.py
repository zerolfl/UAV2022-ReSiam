...
from libs.processing.preprocessing import numpy_to_torch
from .plotting import show_image_with_boxes, overlay_mask
...
class VisHeatmap(VisBase):
    ...
    def draw_data(self, **kwargs):
        ...
        if 'opts' in kwargs:
            opts_new = kwargs['opts']
            self.visdom.heatmap(self.raw_data.clone(),  opts=opts_new , win=self.title)
        else:
            self.visdom.heatmap(self.raw_data.clone(),  opts={'title': self.title}, win=self.title)
...
class VisTextFlush(VisBase):
    ...
    def save_data(self, data):
        if self.raw_data is None:
            ...
            self.raw_data = data + "<br>"+ self.raw_data
...
class VisInfoDictFlush(VisBase):
    def __init__(self, visdom, show_data, title):
        super().__init__(visdom, show_data, title)
        self.raw_data = OrderedDict()
        
        # prefix = "<table style=\"border: 0.5px solid black; margin: 0px 10px\">"
        # a = "<tr style=\"background-color: #015DAA; color: #ffffff\"> <td align=\"left\", width=\"120\", style=\"padding: 4px\"><b>{}</b></td> <td align=\"left\", width=\"100\"><b>{}</b></td> </tr>".format("frame", 100)
        # b = "<tr style=\"background-color: #ebf5ff; color: #000000;\"> <td align=\"left\", width=\"120\", style=\"padding: 4px\">{}</td> <td align=\"left\", width=\"100\">{:.2f}</td> </tr>".format("lasdfar", 0.129)
        # suffix = "</table>"
        
        self.display_text = ""
        self.display_text_pre = "<table style=\"border: 0.5px solid black; margin: 8px 8px;\">"
        
        table_cell_width =200
        
        self.display_text_r1c1_left = "<tr style=\"background-color: #015DAA; color: #ffffff;\"> <td align=\"left\", width=\"{}\", style=\"padding: 2px\">".format(table_cell_width)
        self.display_text_r1c12_mid = "</td> <td align=\"left\", width=\"{}\">".format(table_cell_width)
        self.display_text_r1c2_right = "</td> </tr>"
        
        self.display_text_r2c1_left = "<tr style=\"background-color: #ebf5ff; \"> <td align=\"left\", width=\"{}\", style=\"padding: 2px\">".format(table_cell_width)
        self.display_text_r2c12_mid = "</td> <td align=\"left\", width=\"{}\">".format(table_cell_width)
        self.display_text_r2c2_right = "</td> </tr>"
        
        self.display_text_r3c1_left = "<tr style=\"background-color: #ffffff; \"> <td align=\"left\", width=\"{}\", style=\"padding: 2px\">".format(table_cell_width)
        self.display_text_r3c12_mid = "</td> <td align=\"left\", width=\"{}\">".format(table_cell_width)
        self.display_text_r3c2_right = "</td> </tr>"
        
        self.display_text_last = "</table>"
    ...
    def generate_display_text(self, data):
        ...
        for key, value in data.items():
            ...
            if num_key == 1:
                if value is None:
                    display_text += self.display_text_r1c1_left + '<b>{}</b>'.format(key) + self.display_text_r1c12_mid + '<b>{}</b>'.format('None') + self.display_text_r1c2_right
                elif isinstance(value, (str, int)):
                    display_text += self.display_text_r1c1_left + '<b>{}</b>'.format(key) + self.display_text_r1c12_mid + '<b>{}</b>'.format(value) + self.display_text_r1c2_right
                else:
                    display_text += self.display_text_r1c1_left + '<b>{}</b>'.format(key) + self.display_text_r1c12_mid + '<b>{:.2f}</b>'.format(value) + self.display_text_r1c2_right
            elif num_key % 2 == 0:
                if value is None:
                    display_text += self.display_text_r2c1_left + '{}'.format(key) + self.display_text_r2c12_mid + '{}'.format('None') + self.display_text_r2c2_right
                elif isinstance(value, (str, int)):
                    display_text += self.display_text_r2c1_left + '{}'.format(key) + self.display_text_r2c12_mid + '{}'.format(value) + self.display_text_r2c2_right
                else:
                    display_text += self.display_text_r2c1_left + '{}'.format(key) + self.display_text_r2c12_mid + '{:.2f}'.format(value) + self.display_text_r2c2_right
            elif num_key % 2 == 1:
                if value is None:
                    display_text += self.display_text_r3c1_left + '{}'.format(key) + self.display_text_r3c12_mid + '{}'.format('None') + self.display_text_r3c2_right
                elif isinstance(value, (str, int)):
                    display_text += self.display_text_r3c1_left + '{}'.format(key) + self.display_text_r3c12_mid + '{}'.format(value) + self.display_text_r3c2_right
                else:
                    display_text += self.display_text_r3c1_left + '{}'.format(key) + self.display_text_r3c12_mid + '{:.2f}'.format(value) + self.display_text_r3c2_right
            num_key += 1
        
        return self.display_text_pre + display_text + self.display_text_last

    def save_data(self, data):
        if self.display_text == "":
            self.display_text = self.generate_display_text(data)
        else:
            self.display_text = self.generate_display_text(data) + self.display_text
    
    def draw_data(self):
        display_text = copy.deepcopy(self.display_text)
        self.visdom.text(display_text, opts={'title': self.title}, win=self.title)

class VisInfoDictCurr(VisInfoDictFlush):
    def save_data(self, data):
        self.display_text = self.generate_display_text(data)
...
class VisTracking(VisBase):
    ...
    def draw_data(self):
        ...
        resize_factor = 1
        disp_width = 650
        if max(disp_image.shape) > disp_width:
            resize_factor = disp_width / float(max(disp_image.shape))
...
class Visdom:
    def __init__(self, debug=0, ui_info=None, visdom_info=None):
        ...
        self.visdom.properties(self.blocks_list, opts={'title': '状态栏列表'}, win='block_list')  # CN
    ...
    def register(self, data, mode, debug_level=0, title='Data', **kwargs):
        if title not in self.registered_blocks.keys():
            ...
            elif mode == 'text_flush':
                self.registered_blocks[title] = VisTextFlush(self.visdom, show_data, title)
            elif mode == 'info_dict_flush':
                self.registered_blocks[title] = VisInfoDictFlush(self.visdom, show_data, title)
            elif mode == 'info_dict_curr':
                self.registered_blocks[title] = VisInfoDictCurr(self.visdom, show_data, title)
            elif mode == 'lineplot':
