...
def calc_seq_err_robust(pred_bb, anno_bb, dataset, target_visible=None):
    ...
    if torch.isnan(anno_bb).any():
        if dataset in ['uav', 'UAV123', 'UAV20L', 'UAV123@10fps', 'DTB70', 'UAVDT', 'UAV2022-ReSiam', 'UAV2022-onboard']:
            pass
        else:
...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            ...
            avg_overlap_all[seq_id, trk_id] = err_overlap[valid_frame].mean()  # 平均每帧的overlap
            ...
            ave_success_rate_plot_overlap[seq_id, trk_id, :] = (err_overlap.view(-1, 1) > threshold_set_overlap.view(1, -1)).sum(0).float() / seq_length  # Succ. series
            ave_success_rate_plot_center[seq_id, trk_id, :] = (err_center.view(-1, 1) <= threshold_set_center.view(1, -1)).sum(0).float() / seq_length  # Prec. series
            ave_success_rate_plot_center_norm[seq_id, trk_id, :] = (err_center_normalized.view(-1, 1) <= threshold_set_center_norm.view(1, -1)).sum(0).float() / seq_length  # Norm. Prec. series
